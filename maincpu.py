...
def digit_load(args):
    ...
    dset_loaders["visualizecf"] = DataLoader(test_target, batch_size=1, shuffle=False, num_workers=args.worker,
                                      drop_last=False)
    dset_loaders["visualizetsne"] = DataLoader(test_target, batch_size=1, shuffle=False, num_workers=args.worker,
                                           drop_last=False)
    return dset_loaders
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            print('《SFDA》 cross entropy Loss = {:.2f}  entropy Loss = {:.2f}'.format(classifier_loss, im_loss))
            netF.eval()
            netB.eval()
            acc, _ = cal_acc(dset_loaders['test'], netF, netB, netC)
...
def plot_embedding_2d(X, y, path=None, title=None):
    ...
    for i in range(len(y)):
        plt.text(X[i, 0], X[i, 1], '.', color=plt.cm.Set3(int(y[i, 0])+2), fontdict={'weight': 'bold', 'size': 9})
    plt.xticks([])
    plt.yticks([])
...
def visualizetsne(args):
    ...
    netB.eval()
    netC.eval()
    max_iter = len(dset_loaders["visualizetsne"])
    iter_num = 0
    while iter_num < max_iter:
        try:
            inputs_visual, label_visual, _ = iter_test.next()
        except:
            iter_test = iter(dset_loaders["visualizetsne"])
            inputs_visual, label_visual, _ = iter_test.next()
        inputs_test = inputs_visual
        # features_test = netC(netB(netF(inputs_test)))
        # features_test = netB(netF(inputs_test))
...
def visualizeCF(args, epoch, NF, NB, NC):
    ...
    while iter_num < max_iter:
        try:
            ...
            inputs_visual, label_visual, _ = iter_test.next()
        outputs = NC(NB(NF(inputs_visual)))
        if iter_num == 0:
    ...
    cm.plot(cmap=plt.cm.Reds, number_label=True, normalized=True, plot_lib="seaborn", figsizeline=12, figsizecol=10, size=1, title=CFtitle)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mini_max_epoch', type=int, default=10, help="maximum epoch for train target")
    parser.add_argument('--batch_size', type=int, default=10, help="batch_size")
    parser.add_argument('--worker', type=int, default=0, help="number of workers")
    parser.add_argument('--dschool', type=str, default='CWRU', choices=['CWRU', 'kaggle', 'MFPT'],
                        help='school of dataset')
    parser.add_argument('--dset', type=str, default='l0l2')
    parser.add_argument('--lr', type=float, default=0.1, help="learning rate for train source")
    parser.add_argument('--lr2', type=float, default=0.01, help="learning rate for train target")
    parser.add_argument('--seed', type=int, default=2022, help="random seed")
    parser.add_argument('--cls_par', type=float, default=1.0, help="the weight of cluster loss")
    ...
    parser.add_argument('--drawTSNE', type=bool, default=False)
